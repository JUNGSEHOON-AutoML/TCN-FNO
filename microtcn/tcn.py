    ...
    def forward(self, x, p):
        ...
        if self.conditional:
            if p is not None:
                ...
                # If conditional but no params, use BatchNorm without affine (already in FiLM)
                # For now, just pass through (FiLM's BN is affine=False)
                pass
        else:
            x = self.bn(x)
        x = self.relu(x)
